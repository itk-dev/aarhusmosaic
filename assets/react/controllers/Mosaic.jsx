...
function Mosaic() {
  ...
  const loadTiles = () => {
    ...
    fetch(`/api/v1/tiles${randomPath}?${searchParams}`, {
      ...
      .then((data) => {
        ...
        // If there is no new data, the tiles are shuffled.
        if (JSON.stringify(loadedTiles) === JSON.stringify(tiles)) {
          setTiles([...tiles].sort(() => 0.5 - Math.random()));
        } else {
          // If there is no new data, this is put into state.
          setTiles(
  ...
  useEffect(() => {
    ...
    const gridRows = variant.footerHeight
  ...
  useEffect(() => {
    ...
    const timer = setInterval(
      ...
      config?.variant?.exposeTimeout
        ? config.variant.exposeTimeout * 1000
        : 5000,
    );
    ...
    setNodeRefs((prevNodeRefs) =>
      tiles.reduce((result, element) => {
        ...
        resultCopy[element["@id"]] =
          prevNodeRefs[element["@id"]] || createRef();
        return resultCopy;
      }, {}),
    );
  ...
  return (
    <>
      {errorMessage && <CenteredContent>Error: {errorMessage}</CenteredContent>}
      {!errorMessage && !config && (
        <CenteredContent>
